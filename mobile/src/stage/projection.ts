...
export function wallSurfaces(wall: Extract<StageObject, { type: 'wall' }>): Surface[] {
  ...
  for (let i=0;i<xs.length-1;i++) for (let j=0;j<zs.length-1;j++) {
    ...
    add([[a,-t,c],[b,-t,c],[b,-t,d],[a,-t,d]], '#778995');
    add([[a,t,c],[b,t,c],[b,t,d],[a,t,d]], '#91a2ac');
    if (!filled[i-1]?.[j]) add([[a,-t,c],[a,t,c],[a,t,d],[a,-t,d]], '#596d7b');
    if (!filled[i+1]?.[j]) add([[b,-t,c],[b,t,c],[b,t,d],[b,-t,d]], '#637d8c');
    if (!filled[i]?.[j-1]) add([[a,-t,c],[b,-t,c],[b,t,c],[a,t,c]], '#506572');
    if (!filled[i]?.[j+1]) add([[a,-t,d],[b,-t,d],[b,t,d],[a,t,d]], '#c1ced5');
  }
  return faces;
}
...
export function objectSurfaces(object: StageObject): Surface[] {
  ...
  if (object.type === 'start' || object.type === 'faultLine') {
    ...
    return face([[-width/2,-depth/2,0],[width/2,-depth/2,0],[width/2,depth/2,0],[-width/2,depth/2,0]], object.type === 'start' ? '#dc6547' : '#e8bb36', true);
  }
  if (object.type === 'cardboardTarget' || object.type === 'noShootTarget') {
    const f = activeFaceExtent(object);
    return face([[f.left,0,f.bottom],[f.right,0,f.bottom],[f.right,0,f.top],[f.left,0,f.top]], object.type === 'cardboardTarget' ? '#bc8c50' : '#fffdf6');
  }
  const w = object.geometry.faceWidth, h = object.geometry.faceHeight;
  if (object.type === 'steelPlate') return face([[-w/2,0,0],[w/2,0,0],[w/2,0,h],[-w/2,0,h]], '#a6cbdc');
  ...
  return face([[-0.25,0],[0.25,0],[0.12,0.48],[0.4,0.62],[0.5,0.78],[0.35,0.94],[0,1],[-0.35,0.94],[-0.5,0.78],[-0.4,0.62],[-0.12,0.48]].map(([x,z]) => [x*w,0,z*h]), '#88b8d0');
}
...
export function stageSurfaces(stage: StageDocument): Surface[] {
  return [{ objectId: '', color: '#dce4d3', ground: true, points: [
...
export function stageSvg(stage: StageDocument, yaw: number, zoom: number, selectedId: string | null): string {
  ...
  const polygon = (s: typeof surfaces[number]) => '<polygon points="'+s.projected.map(xy).join(' ')+'" fill="'+s.color+'" stroke="'+(s.objectId === selectedId ? '#007aff' : '#52616a')+'" stroke-width="'+(s.objectId === selectedId ? 2 : 0.45)+'" stroke-linejoin="round"/>';
  let svg = '<svg xmlns="http://www.w3.org/2000/svg" width="960" height="640" viewBox="0 0 960 640"><rect width="960" height="640" fill="#edf2f5"/>'+polygon(surfaces[0]);
  ...
  for (const axis of ['x','y'] as const) {
    ...
    for (let value=60;value<end;value+=60) {
      ...
      svg += '<polyline points="'+xy(projectPoint(a,yaw))+' '+xy(projectPoint(b,yaw))+'" stroke="#b5c3ad" stroke-width="0.7"/>';
    }
  }
  ...
  if (start) { const p=screen(projectPoint(start.position,yaw)); svg+='<text x="'+p.x+'" y="'+p.y+'" text-anchor="middle" fill="#782510" font-size="12">START</text>'; }
  return svg+'</svg>';
}
