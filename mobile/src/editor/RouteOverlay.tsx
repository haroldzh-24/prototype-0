...
export default function RouteOverlay(props: RouteOverlayProps & { stage: StageDocument; transform: ViewportTransform }) {
  ...
  return <>
    {stage.objects.filter(isEngageable).map((target, index) => {
      ...
      return <Text key={target.id} pointerEvents="none" style={{ position: 'absolute', left: point.x + 14, top: point.y - 10, color: colors.accent, backgroundColor: colors.background, fontSize: 11 }}>T{index + 1}</Text>;
    })}
