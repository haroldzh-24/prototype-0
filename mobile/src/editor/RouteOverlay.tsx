...
export type RouteOverlayProps = { assignmentMode?: 'visible' | 'engaged' | null; onTargetTap?: (id: string) => void; route: StageRoute; selectedId: string | null; onSelect: (id: string) => void; onChange: (route: StageRoute) => void; onDragging: (value: boolean) => void };
export default function RouteOverlay(props: RouteOverlayProps & { stage: StageDocument; transform: ViewportTransform }) {
  ...
  return <>
    {selected && stage.objects.filter(t => isEngageable(t) && selected.visibleTargetIds.includes(t.id)).map(target => {
      const a = stageToViewport(selected.position, transform), b = stageToViewport(target.position, transform);
      const engaged = selected.engagedTargetIds.includes(target.id), length = Math.hypot(b.x - a.x, b.y - a.y);
      const color = engaged ? colors.accent : '#52858e';
      return <View key={'connection-' + target.id} pointerEvents="none" style={{ position: 'absolute', left: 0, top: 0 }}>
        <View style={{ position: 'absolute', left: (a.x + b.x) / 2 - length / 2, top: (a.y + b.y) / 2, width: length, height: engaged ? 2 : 1, backgroundColor: color, transform: [{ rotate: Math.atan2(b.y - a.y, b.x - a.x) + 'rad' }] }} />
    ...
    {points.slice(1).map((point, index) => {
      ...
      return <View key={`line-${index}`} pointerEvents="none" style={{ position: 'absolute', left: (a.x + b.x) / 2 - length / 2, top: (a.y + b.y) / 2 - 1, width: length, height: 2, backgroundColor: colors.accent, transform: [{ rotate: `${Math.atan2(b.y - a.y, b.x - a.x)}rad` }] }} />;
    })}
    {!props.assignmentMode && route.positions.map((p, index) => <Marker key={p.id} position={p} index={index} transform={transform} selected={p.id === props.selectedId} select={() => props.onSelect(p.id)} onDragging={props.onDragging} move={position => props.onChange(movePosition(route, p.id, position, stage.stage))} />)}
    {stage.objects.filter(isEngageable).map(target => {
